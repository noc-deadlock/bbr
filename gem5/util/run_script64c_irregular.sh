...
do
	./build/Garnet_standalone/gem5.opt -d $out_dir/64c/irregular/${routing_algorithm[0]}/${conf_file}/${bench_caps[$b]}/vc-${vc_}/inj-${k} configs/example/garnet_synth_traffic.py --topology=irregularMesh_XY --num-cpus=64 --num-dirs=64 --mesh-rows=8 --network=garnet2.0 --router-latency=1 --sim-cycles=${cycles} --inj-vnet=0 --vcs-per-vnet=${vc_} --conf-file=${conf_file} --sim-type=2 --swizzle-swap=1 --policy=1 --tdm=1 --injectionrate=${k} --synthetic=${bench[$b]} --routing-algorithm=0 &
done
#sleep 300
done
done